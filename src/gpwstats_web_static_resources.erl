-module(gpwstats_web_static_resources).
%% Copy from great tutorial Webmachine + Riak + EarlyDTL
...
provide_content(ReqData, Context) ->
  ...
  case Context#context.response_body of
    undefined -> case file_exists(Context, wrq:disp_path(ReqData)) of
        {true, FullPath} ->
          {ok, Value} = file:read_file(FullPath),
          {Value, ReqData, Context#context{response_body=Value}};
        false -> {error, ReqData, Context}
      end;
    _Body ->
